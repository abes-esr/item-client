      ...
      <v-row justify="space-between" class="mt-4 mb-4 ml-1 mr-1">
        ...
        <v-hover v-slot="{isHovering, props}" v-for="type in types" :key="type">
          <v-col>
            ...
            <v-row>
              <v-col v-bind="props"
                     :class="`btn-perso d-flex justify-center elevation-${isHovering ? 6 : 2} pa-5 ma-1 d-flex`"
          ...
          <div class="ml-3 mr-1" v-if="types.indexOf(type) < types.length-1"
               style="width: 10px; border-left: solid 4px #295494"></div>
        </v-hover>
      </v-row>
    </v-card-text>
  </v-card>
...
const types = [
  {
    type: 'PPN',
    message: 'Numéro de notice :'
  },
  {
    type: 'EPN',
    message: 'Numéro d\'exemplaire :'
  }
];
