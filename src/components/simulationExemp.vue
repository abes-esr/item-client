    ...
    getSimulation() {
      ...
      axios({
        ...
        (error) => {
          ...
          this.alert = true;
          this.alertType = 'error';
          this.alertMessage = `Simulation impossible : ${error.response.data.message}.  <br /> Veuillez réessayer ultérieurement. Si le problème persiste merci de contacter l'assistance.`;
          if (error.response.status === 401) {
            this.$emit('logout');
